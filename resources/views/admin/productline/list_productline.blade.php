      ...
      <table class="table table-striped b-t b-light">
        ...
        <tbody id="myTable">
                ...
                <tr>
                    ...
                    <td><span class="text-ellipsis">{{$productline->name}}</span></td>
                    <td>{{$productline->brands->name}}</td>
                    <td width="10px">
                        <a href="{{route('productline.edit',['productline'=>$productline->id])}}">
                            <i class="fa fa-wrench text-success text-active"></i>
                        </a>
                    </td>
                    <form action="{{route('productline.destroy',['productline'=>$productline->id])}}" width="10px" method="post">
                        @csrf
                        @method('delete')
                        <td width="10px">
                            <button type="submit">
