      ...
      <table class="table table-striped b-t b-light">
        ...
        <tbody id="myTable">
                ...
                <tr>
                    <td></td>
                    <td>{{$bill->id}}</td>
                    <td><span class="text-ellipsis">{{$bill->total_price}}</span></td>
                    <td><span class="text-ellipsis">{{$bill->created_at}}</span></td>
                    <td width="10px">
                        <a href="{{route('bill.show',['bill'=>$bill->id])}}" class="active" ui-toggle-class="">
                            <i class="fa fa-eye text-dark text-active"></i>
                        </a>
                    </td>
                   
                    <form action="{{route('bill.destroy',['bill'=>$bill->id])}}" width="10px" method="post">
                        @csrf
                        @method('delete')
                        <td width="10px">
                            <button type="submit">
